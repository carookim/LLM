...
class SpecializedAgent:
    ...
    def process_inbox(self) ->list[Dict[str,Any]]:
        '''받은 메세지 처리'''
        self.set_state(AgentState.PROCESSING) # set_state의 return값이 없음
        results = []
...
if __name__ == '__main__':
    ...
    math_agent = MatchAgent('mathbot','calculate')
    analyzer_agent = DataAnalyzerAgent('analyzerbot','data analysis')
    
    # 코디 네이터(조정자)에 등록
    corrdinator = Corrdinator()
    corrdinator.register_agent(text_agent)
    corrdinator.register_agent(math_agent)
    corrdinator.register_agent(analyzer_agent)
    ...
    # 메세지 라우팅
    # 에이전트아이디에 해당하는 메세지를 해당 에이전트의 inbox에 저장
    corrdinator.route_message()
    
    # 메세지 처리
    corrdinator.process_all_agents()
    
    # 상태 출력
    corrdinator.system_status()