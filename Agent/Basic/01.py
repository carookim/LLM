...
# task_status = AgentState.RUNNING
# print(task_status)
# print(task_status.name)
# print(task_status.value)

# ---
# 에이전트 사용의 basic 방법에 대해서 알아보자.
...
class ExcutionRecord:
    ...
    timestamp:str
    action:str
    status:str # AgentState.PROCESSING -> 'processing' 문자열이 들어옴
    duration:float
...
class SimpleAgent:
    ...
    def set_state(self,state:AgentState):
        '''현재 상태 변경'''
        self._state = state
        print(f'[SUCCESS] [{self._state.name}] 상태 변경 : {self._state.value}')
    # 작업 실행
    def excute(self, input_data:Dict[str,Any]) -> Dict[str,Any]:
        ...
        try:
            ...
            result = self._process(action, input_data) # 작업 처리 메소드 실행
            # 성공 처리
            self.set_state(AgentState.COOMPLETED)
    ...
    def _process(self, action:str,input_data:Dict[str,Any]) -> Any:
        '''실제 작업 수행'''
        if action == 'greet':
            name = input_data.get('name','Guset')
            return f'Hello, {name}'
        elif action == 'add' :
