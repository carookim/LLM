...
class  VectorDBAgent(SpecializedAgent):
    ...
    def __init__(self, name:str):
        ...
        self.client = chromadb.Client(Settings(
            anonymized_telemetry=False, # 익명의 사용정보 수집 및 전송 비활성화
    ...
    def _initialize_db(self):
        ...
        for movie in SAMPLE_MOVIES:
            ...
            self.collection.add(
                ids=[movie['id']],
                documents = [doc_text],
                embeddings = [self._get_embedding_db(doc_text)],
                metadatas = [{
                    'title': movie['title'],
                    'year': movie['year'],
    ...
    def _handle_message(self, message:Message)->Dict[str, Any]:
        ...
        query_embedding = self._get_embedding_db(query)
...
class LLMAgent(SpecializedAgent):
    ...
    def _handle_message(self, message:Message)->Dict[str, Any]:
        ...
        # 컨텍스트 정리
        context_text = '\n'.join([content.get('content',item) for item in context])
