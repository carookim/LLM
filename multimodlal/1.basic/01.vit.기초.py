...
def patch_embedding():
    ...
    도트 하나씩 해서 옮기는것보다 좀 더 큰 단위의 패치크기로 원본이미지를 가져와 효율성을 높이고
    그 각 패치들을 flatten하는 과정'''
    # 설정
    ...
    print(f"패치수 : {image_size//patch_size} x {image_size//patch_size}")
    
    # 더미 이미지 생성
    dummy_image = torch.randn(1, channels, image_size, image_size)
    # 1 → 배치 크기 (batch size)
    # channels → 채널 수 (예: RGB면 3)
    # image_size → 데이터 길이 / 해상도 (수업 맥락에선 입력 차원)
    # 결과:
    # (1, channels, image_size) 형태의 텐서를
    # 평균 0, 표준편차 1인 정규분포로 랜덤 생성하는 것
    
    print(f"더미 이미지 생성")
    print(f"입력 이미지 shape :  {dummy_image.shape}") # [1,3,224,224]
        
    # 패치 분할 (Conv2d 사용)
    # Conc2d stride = patch_size_ 겹치지않는 패치 추출
...
def cls_token():
    embedding_dim = 768
    num_patches = 196
    batch_size = 2 # 두개의 이미지를 동시에 
    # cls 토큰 생성
    cls_token = nn.Parameter(torch.rand(1,1,embedding_dim)) # 현재는 랜덤값으로 벡터 설정을했지만, 학습을 통해 이미지를 요약하는 벡터로 진화
    print(f"CLS 토큰 shape : {cls_token.shape}") # [1,1,196]
    # 배치크기에 맞게 CLS 토큰 확장
    cls_tokens = cls_token.expand(batch_size,-1,-1) # 배치마다 동일한 cls토큰이 필요, expand는 view만 확장
    # 패치 임베딩 생성
...
#    main()
    ...
    url = 'C:\KIM\LLM\multimodlal\1.basic\Cat03.jpg'
