...
if __name__=='__main__':
    ...
    file_path ="C:\\KIM\\LLM\\multimodlal\\1.basic"
    file = glob(file_path+'/*.jpg')
    print(file)
    for f in file: # 이렇게 glob로 추출한 파일은 리스트 안에 있으므로 for문이나 인덱싱으로 추출해야된다. 
    ...
    # smaple 이미지확보
    # hf 방식에서 받은 모델, timm에서 받은 모델로 전용 추론 함수에 넣어서 결과를 확인
    # ex) classify_image_timm(model,)
    
    # tim 모델 사용
    result_1st, top5_indices = classify_image_timm(timm_model, timm_process, image)
    print(f"timm 모델 예측결과 1순위 : {result_1st}")
