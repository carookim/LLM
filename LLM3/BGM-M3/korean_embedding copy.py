...
# 임베딩 모델 테스트
embedding_models = [KoreanEmbeddingModels.get_openai(),KoreanEmbeddingModels.get_bge_m3(),KoreanEmbeddingModels.get_korean_roberta(),KoreanEmbeddingModels.get_multilingual_e5()]
embeding_model_names = ('openai','bge_me', 'ko-roberta','multilingual-e5')

def evaluate_embedding_models(embedding_model:KoreanEmbeddingModels,test_texts:List[str]):
    start_time = time()
    openai_vectors = embedding_model.embed_documents(test_texts)
    elapsed = time() - start_time
    print('openai 임베딩')
    print(f'벡터차원 : {len(openai_vectors[0])}')
    print(f'처리시간 : {elapsed:.2f}')

# 각 임베딩 모델 평가
for model in embedding_models:
    evaluate_embedding_models(model, test_texts)
...
# 하이브리드 검색 - 위에서 한국어 임베딩중에 성능이 가장 좋은 모델을 선택해서
# sparse vector 방식과 하이브리드 로 연결 즉 BM25
# RRF 로 rank 형태로 최종결과를 도출, 혹은 가중치 BM25 retriever와 assemble retriever로 최종 구현(두방식에 대한 가중치를 조정)

# 적절한 프롬프트와 LLm을 체인방식으로 결합해서 결과를 도출
# 또는 lang graph로 구현해도 된다.