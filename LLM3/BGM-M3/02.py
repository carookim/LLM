...
# 이 조건문은 현재 실행되는 파일이 메인 프로그램일 때만 아래 코드를 실행하라는 뜻이야.
# 이 파일을 Python에서 직접 실행하면 True가 됨 → 아래 함수들이 실행됨
# 다른 파일에서 import 해서 가져오면 False가 됨 → 아래 함수 실행 안 됨
# 👉 이 파일을 단독 실행할 때만 테스트/메인 루틴을 실행하도록 해주는 Python의 관례
    # 함수	역할
# check_evnironment()	실행 환경/패키지 체크
# embedding_basic()	텍스트 임베딩 생성 테스트
# cosine_similarity()	임베딩 간 유사도 계산
# bm25_sparse_search()	키워드 기반 BM25 검색 테스트
