...
# 상태업데이트(시뮬레이션 가정)
# 상태 업데이트 + 기본 상태 설정상태로 초기화
state = initial_state.copy()

# 1. 검색노드가 문서를 추가
state['documents'] = [
    Document(page_count='RAG는 검색 증강 생성입니다.',metadata={'source':'wiki'}),
    Document(page_count='RAG는 LLM의 한계를 극복합니다.',metadata={'source':'blog'}),
]
#  Document(page_count= 그냥 임의의 내용 채우기

# 2. 생성노드가 답변을 생성
state['context'] = '\n'.join([doc.page_content for doc in state['documents']])
state['answer'] = 'RAG는 검색 증강 생성 기술입니다.'
#state['context'] = '\n'.join..., state['answer'] = 'RAG는 검색 임의의 내용 채우기
