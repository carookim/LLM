...
def check_enviroment():
    ...
    if not os.getenv('LANGCHAIN_API_KEY'):
        missing_keys.append('LANGCHIAN_API_KEY')
        if missing_keys:
...
def langsmith_client():
    ...
    print('\n최근 실행기록')
    try:
        project_name = os.getenv('LANGCHIAN_PROHECT','default')
        runs = list(client.list_runs(
            project_nname=project_name,
            limit=5
        ))
        if runs:
            for run in runs:
                status = 'success' if run.status == 'success' else 'faile'
                duration = f'{run.total_time:.2f}' if run.total_time else 'NaN'
                print(f'    {status} {run.name}  |  {duration}')
    except Exception as e:
       print(f'최근 실행 기록 조회중 오류 발생 : {e}') 
    print('\n langSmith Client 사용 완료')

if __name__ =='__main__':
    check_enviroment()  #  환경체크
    auto_tracing() # 자동 추적
    traceable_decorater() # 커스텀 함수 추적
